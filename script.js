...
function selectCommissionType(type) {
    ...
    if (type === 'comic') {
        ...
        numCharsGroup.style.display = 'none';
        numCharsInput.removeAttribute('required');
        numCharsInput.value = '';
        typeInput.value = 'comic';
    } else {
        btnIllustration.classList.add('active');
        btnComic.classList.remove('active');
        numCharsGroup.style.display = 'block';
        numCharsInput.setAttribute('required', 'required');
        typeInput.value = 'illustration';
    }
}
